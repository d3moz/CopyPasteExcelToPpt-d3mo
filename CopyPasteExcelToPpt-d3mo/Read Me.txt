...
SlideBot does the job with a little help.  This bot is only useful if your Excel charts/graphs are always in the same location, such as a monthly deck.  They get pasted to a PPT template file.

To add additional slides:

1) Copy template & paste to new taskbot

2) Address all <<<<<>>>>> call outs.  <<<<<>>>>> in the comments indicates areas where you must input something such as a height, vertical position, one or more [Page Down] keystrokes, etc. 
